...
DEFAULT_FEATURES = {
    ...
    "client_programs_tab": {
        "label": _lazy("Client Programs Tab"),
        "description": _lazy("Split the participant detail view into separate Profile and Programs tabs for agencies that want enrolment details separated from the core profile."),
        "when_on": [_lazy("The Info tab is relabelled to Profile"), _lazy("A dedicated Programs tab appears on participant records"), _lazy("Program enrolments and episode details move into the Programs tab")],
        "when_off": [_lazy("Participant records keep the existing Info tab layout"), _lazy("Program enrolments remain embedded in the Info tab")],
        "depends_on": ["programs"],
        "used_by": [],
    },
    "group_relationship_column": {
        "label": _lazy("Group Relationship Column"),
        "description": _lazy("Show relationship information from Circles in group rosters when that context is available to the current user."),
        "when_on": [_lazy("Group rosters show a Relationship column"), _lazy("Relationship labels come from visible Circle memberships only")],
        "when_off": [_lazy("Group rosters keep the existing columns"), _lazy("No relationship data is shown in group rosters")],
        "depends_on": ["groups", "circles"],
        "used_by": [],
    },
    "attendance_navigation": {
        "label": _lazy("Attendance Navigation"),
        "description": _lazy("Add a dedicated Attendance menu item that routes staff to session logging and attendance reports without changing the default Groups workflow for other tenants."),
        "when_on": [_lazy("Attendance appears as its own navigation item"), _lazy("Staff can open a dedicated attendance hub with session and report actions")],
        "when_off": [_lazy("Attendance remains accessed through group detail pages"), _lazy("The existing Groups navigation stays unchanged")],
        "depends_on": ["groups"],
        "used_by": [],
    },
    "quick_notes": {
