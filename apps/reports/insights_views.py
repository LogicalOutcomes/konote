...
def _build_metric_context(program, date_from, date_to, active_themes):
    """Build metric distribution, achievement, and summary card context.

    Separated from the view to keep program_insights() readable.

    Returns:
        dict of template context variables for metric sections.
    """
...
def program_insights(request):
    ...
    if form.is_bound and form.is_valid():
        ...
        if data_tier != "sparse":
            metric_ctx = _build_metric_context(program, date_from, date_to, active_themes)

        # ── Auto-expand logic (DRR §5) ──
        expand_participant_voice = True  # Default: always open
        expand_distributions = bool(metric_ctx["metric_distributions"])
        expand_outcomes = bool(metric_ctx["achievement_rates"])
        expand_staff_assessments = bool(structured.get("descriptor_trend"))
        expand_engagement = False  # Default: collapsed

        if metric_ctx["has_urgent_themes"]:
            # Urgent feedback → Participant Voice always open
            expand_participant_voice = True
        elif metric_ctx["metric_distributions"] or metric_ctx["achievement_rates"]:
