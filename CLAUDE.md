...
- `tasks/design-rationale/fhir-informed-modelling.md` — FHIR-informed data modelling. "Borrow concepts, don't comply" approach. ServiceEpisode (extends ClientProgramEnrolment in place), Goal achievement status, unified Outcome Domain taxonomy, encounter participant roles. Anti-patterns (FHIR server, mandatory presenting issues at intake, worker-selected episode type). Graduated complexity path for referrals, care teams, explicit presenting issues.
