...
- `tasks/design-rationale/self-hosted-llm-infrastructure.md` — Shared Ollama VPS for KoNote + OpenWebUI + survey analysis. Qwen3.5-35B-A3B (MoE, 3B active) on OVHcloud VPS-4 (48 GB RAM). Dual-provider architecture (OpenRouter for tools-only, self-hosted for participant data). Docker Compose stack with Caddy auth. Anti-patterns (exposing Ollama directly, participant PII in OpenWebUI, GPU at current scale).
