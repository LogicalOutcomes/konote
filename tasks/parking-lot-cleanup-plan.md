...
Clear 16 actionable items from the parking lot in one coordinated push. Items are grouped into 5 bundles by file area to minimise merge conflicts when running parallel sessions. Each bundle touches a distinct set of files — bundles 1–3 and 5 can run simultaneously as sub-agents.

INSIGHTS-I18N1 (from Active Work, not the parking lot) is included in Bundle 3 because it's the same type of work (locale files).

---

## Items Deferred (Need More Information)

These 25 items stay in the parking lot — they need GK review, design work, or prerequisites that aren't ready yet:

| ID | Reason to Defer |
|---|---|
| QA-PA-ERASURE1 | GK reviews privacy/data retention language |
| QA-R7-PRIVACY1 | GK reviews PIPEDA data export workflow design |
| QA-R7-PRIVACY2 | GK reviews consent withdrawal workflow design |
| QA-R7-EXEC-COMPLIANCE1 | GK reviews reporting methodology |
...
| SURVEY1 | Very large feature — models exist but views/forms/templates are weeks of work |
...
### Task 2.3 — DEPLOY-VERIFY1: Verify deploy-azure.md reference
...
**Order within bundle:** Do 3.1 (stale cleanup) first, then 3.2 and 3.3. Cleaning stale entries first means fewer entries to scan when adding new ones.
...
| Agent B | Bundle 2 (Verification) | QA-R7-BUG13, QA-R7-BUG21, DEPLOY-VERIFY1 | 1 hour |
| Agent C | Bundle 3 (i18n) | I18N-STALE1, I18N-CSV1, INSIGHTS-I18N1 | 3 hours |
...
No cross-bundle dependencies. All bundles are independent.
