...
**Date:** 2026-02-21
**Status:** Steps 1-5 implemented (form, views, URL, export engine, HTMX cascade, tests). Steps 6-10 pending.
...
| **Ad-hoc export** | `/reports/export/` | Program managers and admins only | Data extraction with full manual control |

Executives ONLY see the template-driven path. No fallback to ad-hoc.

## Entry Points

| From | Action | Destination |
|------|--------|-------------|
| **Navigation menu** | "Reports" link | `/reports/generate/` (executives); both paths for PMs |
...
> "No report templates have been set up yet. Report templates define what metrics and demographic breakdowns to include. Contact your administrator to configure one, or ask the Program Manager for a custom data export."

Do NOT fall back to the ad-hoc form. Do NOT show metric checkboxes.
...
**This is why templates must be the only report path for executives.** Manual metric selection makes cross-agency reports impossible because each agency would choose different metrics. The template guarantees consistency across consortium members.
...
| 9 | Update navigation: executives -> generate, PMs -> both | Steps 3, 8 |
...
- **Falling back to ad-hoc for executives** — defeats the template architecture, creates inconsistent reports
