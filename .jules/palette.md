...
## 2025-03-10 - Explicit type="button" on Tab/Filter Controls
**Learning:** HTMX-powered or JS-controlled generic buttons (like `tab-btn` or `filter-btn`) default to `type="submit"` if not explicitly set. This can cause confusing accessibility behavior and unintended form submissions.
**Action:** Always explicitly define `type="button"` on interactive UI elements that aren't submitting a form.
