...
## 2024-05-14 - [Login CSRF on Demo Endpoints]
**Vulnerability:** The `demo_login` and `demo_portal_login` views had the `@csrf_exempt` decorator applied.
**Learning:** This introduces a Login CSRF vulnerability, allowing an attacker to log a victim into an attacker-controlled account, which can then be used to capture the victim's activity or data.
**Prevention:** Never use `@csrf_exempt` on authentication boundaries (login, demo login, invite accept, password reset) unless absolutely required by an external system callback. Ensure frontend forms include `{% csrf_token %}`.
