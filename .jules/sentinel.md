...
## 2026-05-07 - [CSRF Missing Protection on Demo Login Endpoints]
**Vulnerability:** The `demo_login` and `demo_portal_login` endpoints in `apps/auth_app/views.py` were decorated with `@csrf_exempt`, despite being `@require_POST` and handling user authentication. This exposed them to Login CSRF attacks, where a malicious site could force a user's browser to authenticate them into an attacker-controlled demo session.
**Learning:** Authentication boundaries, even those used for "demo" or "quick login" purposes, must always enforce CSRF protection to ensure session integrity and prevent forced logins.
**Prevention:** Never use `@csrf_exempt` on any endpoint that establishes an authenticated session (e.g., calling `login(request, user)`). Ensure all frontend authentication forms include `{% csrf_token %}` and rely on standard Django CSRF middleware validation.
