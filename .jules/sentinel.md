...
## 2024-03-05 - [Login CSRF on Demo Authentication Endpoints]
**Vulnerability:** The `demo_login` and `demo_portal_login` endpoints in `apps/auth_app/views.py` were decorated with `@csrf_exempt`, allowing Cross-Site Request Forgery (CSRF) on login actions.
**Learning:** Authentication boundaries, even those for demo purposes, must not bypass CSRF protection because an attacker could force a victim to log in as a demo user, potentially tracking the victim's actions or causing confusion. The `login.html` template already included the required `{% csrf_token %}`, making the `@csrf_exempt` unnecessary and dangerous.
**Prevention:** Avoid using `@csrf_exempt` on authentication boundaries. Ensure forms submitting to login endpoints always include `{% csrf_token %}` and rely on standard Django CSRF middleware.
