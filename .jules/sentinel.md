...
## $(date +%Y-%m-%d) - [Missing CSRF Protection on Demo Login]
**Vulnerability:** The `demo_login` and `demo_portal_login` endpoints in `apps/auth_app/views.py` were annotated with `@csrf_exempt`, removing Django's built-in Cross-Site Request Forgery (CSRF) protection for these POST requests. This introduced a Login CSRF vulnerability where an attacker could force a user's browser to log into a demo account.
**Learning:** Even though demo endpoints exist for testing or showcase purposes, they represent an authentication boundary and must be secured to the same standard as real login endpoints. Exempting them from CSRF protection violates the "defense in depth" principle and introduces risk, especially if demo credentials have any elevated access in the demo environment.
**Prevention:** Avoid using `@csrf_exempt` on any authentication boundaries (e.g., login, demo login, invite accept). Rely on Django's default CSRF middleware and ensure all frontend forms submitting to these endpoints include the `{% csrf_token %}` tag.
