...
## 2026-05-15 - [Login CSRF Vulnerability on Demo Endpoints]
**Vulnerability:** The `demo_login` and `demo_portal_login` views in `apps/auth_app/views.py` had the `@csrf_exempt` decorator applied, which made them vulnerable to Login CSRF attacks. An attacker could forcefully log a user into an attacker-controlled account, or one of the demo accounts, potentially tricking them into revealing sensitive information or tracking their actions.
**Learning:** Applying `@csrf_exempt` to authentication boundaries is dangerous unless explicitly required by external callback mechanisms (such as SAML or OIDC postbacks). Since the frontend forms in `templates/auth/login.html` already include the `{% csrf_token %}` tag, there is no need for these endpoints to bypass CSRF protection.
**Prevention:** Avoid using `@csrf_exempt` on authentication boundaries (e.g., login, demo login, invite accept, password reset) unless absolutely required by an external system callback. Ensure forms in templates include `{% csrf_token %}`.
