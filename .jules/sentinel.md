...
## 2024-03-05 - [CSRF Exemption on Demo Logins]
**Vulnerability:** The `@csrf_exempt` decorator was used on state-changing authentication endpoints (`demo_login` and `demo_portal_login`), exposing the application to Login CSRF vulnerabilities.
**Learning:** Removing standard CSRF protection from authentication endpoints, even for "demo" users, allows attackers to force a victim to authenticate as the demo user. If the application already has the `{% csrf_token %}` included in the frontend login forms, these decorators provide no benefit and only reduce security.
