...
## 2024-05-24 - Login CSRF on Demo Authentication Boundaries
**Vulnerability:** The `demo_login` and `demo_portal_login` views in `apps/auth_app/views.py` were decorated with `@csrf_exempt`, disabling CSRF protection on authentication endpoints.
**Learning:** Authentication boundaries, even demo or testing ones, must retain CSRF protection to prevent Login CSRF attacks where an attacker forcefully logs a victim into an attacker-controlled account or demo environment. The frontend forms were correctly supplying `{% csrf_token %}`, so the exemption on the backend was unnecessary and dangerous.
**Prevention:** Avoid using `@csrf_exempt` on any endpoint that authenticates a user or establishes a session unless strictly required by an external system callback. Always verify if the corresponding frontend forms include the token before removing the protection.
